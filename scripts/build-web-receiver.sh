...
# The glue + wasm live at stable URLs but are cached by browsers/CDN. A
# content-hash version module lets the frontend append ?v=<hash> so any
# engine change busts caches immediately (a stale glue once shipped
# "this.inner.begin_file is not a function" to returning visitors).
echo "==> writing src/lib/webrxVersion.ts (cache-busting content hash)"
HASH=$(cat "$OUT/web_receiver.js" "$OUT/web_receiver_bg.wasm" | sha256sum | cut -c1-12)
cat > "$ROOT/src/lib/webrxVersion.ts" <<EOF
...
export const WEBRX_VERSION = "$HASH";
EOF
